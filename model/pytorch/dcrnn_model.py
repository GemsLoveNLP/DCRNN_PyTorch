...
class EncoderModel(nn.Module, Seq2SeqAttrs):
    ...
    def __init__(self, adj_mx, **model_kwargs):
        ...
        self.dropout = float(model_kwargs.get('dropout', 0.0))  # <-- Add a dropout attribute here
    ...
    def forward(self, inputs, hidden_state=None):
        ...
        for layer_num, dcgru_layer in enumerate(self.dcgru_layers):
            ...
            # Apply dropout after each DCGRU layer’s output
            next_hidden_state = F.dropout(next_hidden_state, p=self.dropout, training=self.training)

            hidden_states.append(next_hidden_state)
            output = next_hidden_state  # feed to next layer
